...
class StyleGAN2Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, no_antialias=False, size=None):
        super().__init__()

        MIN_CROP_LOAD_SIZE = 256
        FINAL_RES_LOG2 = 2

        self.stddev_group = 16
        if size is None:
            size = 2 ** int((np.rint(np.log2(MIN_CROP_LOAD_SIZE))))
...
class StyleGAN2Encoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False):
        super().__init__()
        ...
        cur_res = 2 ** int((np.rint(np.log2(MIN_CROP_LOAD_SIZE))))
        convs = [nn.Identity(),
...
class StyleGAN2Decoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False):
        ...
        num_downsampling = NUM_DOWNSAMPLING
        cur_res = 2 ** int((np.rint(np.log2(MIN_CROP_LOAD_SIZE)))) // (2 ** num_downsampling)
        convs = []
